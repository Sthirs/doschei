...
- Users can view a group's spend for one month broken down by category family, with each family's amount and share of the month's total shown side by side (including families with no spend that month), always listed in the same order regardless of amount, plus the month's total, its expense count, and their own share of it. Users can move the selected month backwards and forwards one month at a time.
...
- The category recap follows the same rules as monthly spend totals: settle-up entries are excluded, a member's own share is their split total, and the selected month starts at the current month and never extends into the future. An expense whose category key is not recognized is counted under Uncategorized.
...
- Architecture claims that are fully implemented (Express, TypeORM, dedicated
  ...
  purge on new deploy, the group monthly totals chart, and the group category
  recap) are not annotated here.
